...
def world_with_walls() -> World:
    ...
    for x in range(10):
        world.set_tile(Tile(position=Position(x=x, y=5), walkable=False, opaque=True))
    return world
...
def two_entities(empty_world: World) -> World:
    ...
    empty_world.add_entity(Entity(entity_id="entity_a", position=Position(x=2, y=2)))
    empty_world.add_entity(Entity(entity_id="entity_b", position=Position(x=7, y=7)))
    return empty_world
...
def adjacent_entities(empty_world: World) -> World:
    ...
    empty_world.add_entity(Entity(entity_id="entity_a", position=Position(x=3, y=3)))
    empty_world.add_entity(Entity(entity_id="entity_b", position=Position(x=4, y=3)))
    return empty_world
...
def three_entities_triangle(empty_world: World) -> World:
    ...
    Positions:
        ...
        c at (4, 4)
    """
    empty_world.add_entity(Entity(entity_id="a", position=Position(x=3, y=3)))
    empty_world.add_entity(Entity(entity_id="b", position=Position(x=4, y=3)))
    empty_world.add_entity(Entity(entity_id="c", position=Position(x=4, y=4)))
    return empty_world
