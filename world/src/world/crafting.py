"""Crafting: the craft phase over the shared recipe table.

The table itself is `bobgame_rules.recipes` (repo root `rules/`), which the
agents read too; `Recipe`, `RECIPES`, `recipe_cost_text` and
`CRAFT_PROGRESS_PREFIX` are re-exported here under the names the rest of
`world` already imports.
"""
...
from bobgame_rules.recipes import (
